...
function commitData(cQty, cCreate, oID, oUser) {
  $('#furfillment').text(cQty);
  console.log(cQty);
}
...
function addOffering(
  ...
  offerFurfill.text('Furfillment:' + ' out of ' + oFurfillment);
...
db.ref('commits').on('child_added', function(snapshot) {
  commitSnapshot = snapshot.val();
  console.log(commitSnapshot);

  //var hold2 = commitSnapshot.commitQty;

  commitData(
    commitSnapshot.commitQty,
    commitSnapshot.createDate,
    commitSnapshot.offeringID,
